...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        # super().__init__() 也可以
        # super().__init__()的作用也就显而易见了，就是执行父类的构造函数，
        # 使得我们能够调用父类的属性
    ...
    def forward(self, x):
        x = F.max_pool2d(F.relu(self.conv1(x)),
                         (2, 2))  # 输入x经过卷积conv1之后，经过激活函数ReLU（原来这个词是激活函数的意思），使用2x2的窗口进行最大池化Max pooling，然后更新到x。
    ...
    def num_flat_features(self, x):
        ...
        # 除去批处理维度的其他所有维度
        # nn.Conv2d 接受一个4维的张量，即[nSamples x nChannels x Height x Width] 除去第一个，
        num_features = 1
        for s in size:
            num_features *= s
        # num_feature = nChannels x Height x Width
        return num_features


net = Net()     # 实例化一个对象net，以后调用的时候要用net()
print(net)
