...
def cli(ctx: click.Context,
    ...
    path = "/home/tongping/satellite_Image/satellite_middle.tif"
    image = cv2.imread(path)
    image = np.transpose(image, (2, 0, 1))
    data = torch.from_numpy(image).cuda(device=in_device)  # H*W*C

    dataset_size = 4
    train_data = data.unsqueeze(0).repeat(dataset_size, 1, 1, 1).float().cuda(device=in_device)
    train_target = torch.zeros(dataset_size, 1).float().cuda(device=out_device)
    ...
    def run_epoch(epoch: int) -> Tuple[float, float]:
        ...
        for i, (input, target) in enumerate(train_dataloader):
            ...
            percent = (i+1) / len(train_dataloader) * 100
